...
float find_SGPA(float arr_gpa[],int arr_credit_hours[], int n)
{
    float sum = 0;
    for (int i = 0; i < n; i++)
    {
        arr_gpa[i]*=arr_credit_hours[i];
        sum+=arr_gpa[i];
    }
    return sum;
...
int find_credit_hours(int arr_credit_hours[], int n)
{
    int total_credit_hours=0;
    for (int i = 0; i < n; i++)
    {
        cin>>arr_credit_hours[i];
        total_credit_hours+=arr_credit_hours[i];
    }
...
int main()
{
    ...
    while(sem!=x+1)
    {
        cout<<"How many Subjects are in Semester "<<sem<<" : \n";
        int n = get_arr_size();
        float arr_marks[n];
        ...
        //Entering Credit hours
        cout<<"Start entering credit hours of each subject : ";
        int total_credit_hours=find_credit_hours(arr_credit_hours, n);
        ...
        //Finding Gpa
        cout<<"Your Detailed GPA is : \n";
        find_gpa(arr_avg_marks, arr_gpa, n);
        display_gpa(arr_gpa, n);
        
        //Finding SGPA
        cout<<"Your SGPA is : \n";
        SGPA = find_SGPA(arr_gpa,arr_credit_hours, n);
        SGPA/=total_credit_hours;
        cout<<"SGPA of Semester "<<sem<<" is : "<<SGPA<<endl;
        sum_SGPA=sum_SGPA+SGPA;
        float CGPA = sum_SGPA/sem;
        cout<<"CGPA of Semester "<<sem<<" is : "<<CGPA<<endl;
        sem++;
    }
}